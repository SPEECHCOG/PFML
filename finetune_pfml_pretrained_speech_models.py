...
Code for fine-tuning a PFML pre-trained neural network using randomly generated speech data.
The fine-tuning is run in two stages:
...
if len(sys.argv) > 2:
    sys.exit('\nUsage: \n1) python finetune_pfml_pretrained_speech_models.py \nOR \n2) python finetune_pfml_pretrained_speech_models.py <configuration_file>')
if len(sys.argv) == 2:
    conf = SourceFileLoader('', sys.argv[1]).load_module()
    conf_file_name = sys.argv[1]
else:
    try:
        import conf_finetune_pfml_pretrained_speech_models as conf
        conf_file_name = 'conf_finetune_pfml_pretrained_speech_models.py'
    except ModuleNotFoundError:
        sys.exit('\nUsage: \n1) python finetune_pfml_pretrained_speech_models.py \nOR \n2) python finetune_pfml_pretrained_speech_models.py <configuration_file>\n\n' \
                 'By using the first option, you need to have a configuration file named "conf_finetune_pfml_pretrained_speech_models.py" in the same ' \
                 'directory as "finetune_pfml_pretrained_speech_models.py"')
...
speech_loss = getattr(__import__('torch.nn', fromlist=[conf.loss_name]), conf.loss_name)
...
if __name__ == "__main__":
    ...
    Data = []
    for utterance_index in range(conf.num_randomly_generated_utterances):
        
        # We generate random signals to simulate having a set of utterances
        num_samples_in_utterance = int(conf.max_length_seconds * conf.fs)
        x = np.linspace(0, num_samples_in_utterance, num_samples_in_utterance)
        
        if np.random.rand() < 0.5:
            Data.append(np.random.rand() * np.sin(x) + np.random.normal(scale=0.1, size=len(x)))
        else:
            Data.append(np.random.rand() * np.cos(x) + np.random.normal(scale=0.1, size=len(x)))
    
    with open(f'{conf.result_dir}/{conf.name_of_log_textfile}', 'a') as f:
        f.write('Done!\n\n')
    
    # We generate a random vector of class prior probabilities for the simulated data
    prior_prob = np.array([0.334, 0.666])
    ...
    for num_removed_transformer_encoder_layers in range(block_by_block_finetuning_iterations):
        ...
        best_model_encoder_name_finetuned = f'pfml_speech_best_finetuned_encoder_experiment_{conf.finetuning_experiment_number}_{num_transformer_encoder_layers}_layers.pt'
        best_model_timeseries_name_finetuned = f'pfml_speech_best_finetuned_transformer_experiment_{conf.finetuning_experiment_number}_{num_transformer_encoder_layers}_layers.pt'
        
        with open(f'{conf.result_dir}/{conf.name_of_log_textfile}', 'a') as f:
            f.write(f'################## Fine-tuning model, {num_transformer_encoder_layers} Transformer encoder layers ##################\n\n')

        num_utterances = len(Data)
        num_utterances_per_fold = int(np.floor(num_utterances // conf.num_folds))
    
        if conf.randomize_order_kfolds:
            perm = np.random.RandomState(seed=42).permutation(num_utterances)
        else:
            perm = np.arange(num_utterances)
        ...
        for fold in range(conf.num_folds):
            ...
            if fold == conf.num_folds - 1:
                test_inds = perm[fold*num_utterances_per_fold:]
            else:
                test_inds = perm[fold*num_utterances_per_fold:(fold+1)*num_utterances_per_fold]
            ...
            conf_pt.transformer_params['output_channels'] = 2 # Number of output categories
            ...
            for finetuning_stage in [1, 2]:
                ...
                if conf.use_class_weights:
                    loss_function = speech_loss(weight=class_weights, **conf.loss_params)
                else:
                    loss_function = speech_loss(**conf.loss_params)
                ...
                if conf.train_model:
                    ...
                    for epoch in range(1, conf.max_epochs + 1):
                        ...
                        for train_data in train_data_loader:
                            ...
                            Y_pred, _, _, _ = Timeseries_model(Embedding)
                        ...
                        elif conf.train_criterion == 'uar':
                            epoch_accuracy_training = uar_function(epoch_true_Y_training, epoch_pred_Y_training)
                            epoch_accuracy_validation = uar_function(epoch_true_Y_validation, epoch_pred_Y_validation)
                        else:
            ...
            if conf.test_model:
                ...
                with no_grad():
                    ...
                    conf_mat = confusion_matrix(epoch_true_Y_testing, epoch_pred_Y_testing, labels=np.arange(2))
        ...
        with open(f'{conf.result_dir}/{conf.name_of_log_textfile}', 'a') as f:
            ...
            f.write(f'Mean UAR score for {conf.num_folds}-folds cross-validation: {fold_test_accuracies_mean}')
            f.write('\n########################################################################################\n\n\n\n')
