...
Code for pre-training neural networks using PFML for EEG data and for using a
...
if len(sys.argv) > 2:
    sys.exit('\nUsage: \n1) python pfml_pretrain_eeg.py \nOR \n2) python pfml_pretrain_eeg.py <configuration_file>')
if len(sys.argv) == 2:
    conf = SourceFileLoader('', sys.argv[1]).load_module()
    conf_file_name = sys.argv[1]
else:
    try:
        import conf_pfml_pretrain_eeg as conf
        conf_file_name = 'conf_pfml_pretrain_eeg.py'
    except ModuleNotFoundError:
        sys.exit('\nUsage: \n1) python pfml_pretrain_eeg.py \nOR \n2) python pfml_pretrain_eeg.py <configuration_file>\n\n' \
        'By using the first option, you need to have a configuration file named "conf_pfml_pretrain_eeg.py" in the same ' \
        'directory as "pfml_pretrain_eeg.py"')
    ...
    if conf.train_model:
        ...
        for epoch in range(1, conf.max_epochs + 1):
            ...
            for train_data in train_data_loader:
                ...
                X_input, data_masks, target_functionals = [element.to(device) for element in train_data]
            ...
            with no_grad():
                ...
                for validation_data in validation_data_loader:
                    X_input, data_masks, target_functionals = [element.to(device) for element in validation_data]
                    target_functionals = target_functionals.float()
                    padding_masks = data_masks.bool()
                    Embedding = Encoder(X_input.float())
                    X_output, mask_indices = Transformer(Embedding, src_key_padding_mask=padding_masks, mask_embeddings=True)
        ...
        for output_type in [None, 'ff_outputs', 'ff_residual_outputs', 'end_of_block',
            ...
            with no_grad():
                ...
                for test_data in test_data_loader:
                    X_input, data_masks, _ = [element.to(device) for element in test_data]
                    padding_masks = data_masks.bool()
                    Embedding = Encoder(X_input.float())
