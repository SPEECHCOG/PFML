...
Code for pre-training neural networks using PFML for speech data and for using a
PFML pre-trained model to extract features.
...
if len(sys.argv) > 2:
    sys.exit('\nUsage: \n1) python pfml_pretrain_speech.py \nOR \n2) python pfml_pretrain_speech.py <configuration_file>')
if len(sys.argv) == 2:
    conf = SourceFileLoader('', sys.argv[1]).load_module()
    conf_file_name = sys.argv[1]
else:
    try:
        import conf_pfml_pretrain_speech as conf
        conf_file_name = 'conf_pfml_pretrain_speech.py'
    except ModuleNotFoundError:
        sys.exit('\nUsage: \n1) python pfml_pretrain_speech.py \nOR \n2) python pfml_pretrain_speech.py <configuration_file>\n\n' \
        'By using the first option, you need to have a configuration file named "conf_pfml_pretrain_speech.py" in the same ' \
        'directory as "pfml_pretrain_speech.py"')
...
if __name__ == '__main__':
    ...
    if conf.train_model:
        ...
        training_set = pfml_dataset(train_val_test='train', **conf.params_train_dataset)
        train_data_loader = DataLoader(training_set, **conf.params_train)
        with open(conf.name_of_log_textfile, 'a') as f:
            f.write('Done!\n')
            f.write('Initializing validation set...\n')
        validation_set = pfml_dataset(train_val_test='validation', **conf.params_validation_dataset)
    ...
    if conf.extract_features:
        ...
        test_set = pfml_dataset(train_val_test='test', **conf.params_feature_extraction_dataset)
    ...
    if conf.train_model:
        ...
        for epoch in range(1, conf.max_epochs + 1):
            ...
            for train_data in train_data_loader:
                ...
                # Get the batches
                X_input, padding_masks, target_functionals = [element.to(device) for element in train_data]
                target_functionals = target_functionals.float()
            ...
            with no_grad():
                ...
                for validation_data in validation_data_loader:
                    X_input, padding_masks, target_functionals = [element.to(device) for element in validation_data]
                    target_functionals = target_functionals.float()
                    Embedding = Encoder(X_input.float())
                    X_output, mask_indices = Transformer(Embedding, src_key_padding_mask=padding_masks, mask_embeddings=True)
                    pred_functionals = Decoder(X_output)
    ...
    if conf.extract_features:
        with open(conf.name_of_log_textfile, 'a') as f:
            f.write('\n\nExtracting features using a PFML pre-trained model...\n\n')
        ...
        for output_type in [None, 'ff_outputs', 'ff_residual_outputs', 'end_of_block',
            ...
            with no_grad():
                ...
                for test_data in test_data_loader:
                    X_input, padding_masks, _ = [element.to(device) for element in test_data]
                    Embedding = Encoder(X_input.float())
